...
describe('MediaHandler', () => {
  ...
  it('starts preview when file exists', async () => {
    (mockTorrentService as any).getTorrentByUid.mockResolvedValue({ hash: 'h' });
    (mockTorrentService as any).getTorrentFiles.mockResolvedValue([{ index: 0, name: 'a.mkv', size: 1024 }]);
    await handlePreviewCommand(ctx as any, mockTorrentService as any, mockLogger as any);
  ...
  it('starts thumbnail when file exists', async () => {
    ...
    (mockTorrentService as any).getTorrentByUid.mockResolvedValue({ hash: 'h' });
    (mockTorrentService as any).getTorrentFiles.mockResolvedValue([{ index: 0, name: 'a.mkv', size: 1024 }]);
    await handleThumbnailCommand(ctx as any, mockTorrentService as any, mockLogger as any);
  ...
  it('replies error when service throws', async () => {
    (mockTorrentService as any).getTorrentByUid.mockRejectedValue(new Error('fail'));
    await handlePreviewCommand(ctx as any, mockTorrentService as any, mockLogger as any);
